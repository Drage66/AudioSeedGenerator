...
namespace ImGui {
// Creates SliderDouble Class derived from ImGui SliderScalar
...
struct ImSpectrogram : App {
	...
	//NOTE: Samples are stored in a vector float
    ...
    ImSpectrogram(std::string title, int argc, char const *argv[]) :
		// argc and argc are how command line arguments are passed to main().
		// argc(argument count) is the number of strings pointed to by argv(argument vector)
        App(title, WIN_W,WIN_H,argc,argv),
        m_spectrogram(N_FRQ*N_BIN,0)
    {
		// argc(argument count) is the number of strings pointed to by argv(argument vector)
		// so if the argc is less than 2 than file path is then equal to this string if not the file path is equal to our argument set in the command line when running the program
